...
#        qt5 jpeg libpng tiff zlib boost lcms libraw
#
# Usage:
...
if [[ -n "$MXE_DIR" && -d "$MXE_DIR" ]]; then
  ...
  echo "  cd ~/mxe && make MXE_TARGETS=${MXE_TARGET} qt5 jpeg libpng tiff zlib boost lcms libraw" >&2
...
if [[ ! -f "$TOOLCHAIN_FILE" ]]; then
  ...
  echo "  cd $MXE_ROOT && make MXE_TARGETS=${MXE_TARGET} qt5 jpeg libpng tiff zlib boost lcms libraw" >&2
  exit 1
fi
